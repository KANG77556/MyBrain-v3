...
public final class CloudAiWorkItemAnalyzer {
    ...
    public static AnalysisResult analyze(
        ...
        String jsonText = AiSettings.PROVIDER_GEMINI.equals(AiSettings.normalizeProvider(provider))
                ? callGemini(model, credential, prompt)
                : callOpenAi(model, credential, prompt);
        ParsedWorkItem result = mergeJson(jsonText, originalText, localBaseline, zoneId);
        result.aiProvider = AiSettings.normalizeProvider(provider);
        return new AnalysisResult(result, filtered.masked);
    }

    private static String callOpenAi(String model, String credential, String prompt) throws Exception {
        ...
        if (output != null) {
            for (int i = 0; i < output.length(); i++) {
                JSONArray content = output.optJSONObject(i) == null
                        ? null : output.optJSONObject(i).optJSONArray("content");
    ...
    private static String callGemini(String model, String credential, String prompt) throws Exception {
        ...
        if (candidates != null && candidates.length() > 0) {
            ...
            if (parts != null) {
                ...
                for (int i = 0; i < parts.length(); i++) {
                    String value = parts.optJSONObject(i) == null ? "" : parts.optJSONObject(i).optString("text", "");
                    text.append(value);
                }
    ...
    private static ParsedWorkItem mergeJson(
        ...
        if (json.has("allDay") && !json.isNull("allDay")) result.allDay = json.optBoolean("allDay", result.allDay);
        ...
        if (json.has("confidence") && !json.isNull("confidence")) {
            result.confidence = (float) Math.max(0.0, Math.min(1.0, json.optDouble("confidence", result.confidence)));
        } else {
            result.confidence = Math.max(result.confidence, 0.82f);
        }
        if (result.startAt != null && result.endAt != null && result.endAt <= result.startAt) {
            result.endAt = result.startAt + 3_600_000L;
        }
        return result;
    }
    ...
    private static JSONObject responseSchema() throws Exception {
        JSONObject properties = new JSONObject()
                .put("type", new JSONObject().put("type", "string").put("enum", new JSONArray().put("SCHEDULE").put("TASK").put("MEMO")))
                ...
                .put("repeatRule", new JSONObject().put("type", "string").put("enum", new JSONArray().put("NONE").put("DAILY").put("WEEKDAYS").put("WEEKLY").put("MONTHLY")))
                .put("priority", new JSONObject().put("type", "string").put("enum", new JSONArray().put("LOW").put("NORMAL").put("HIGH")))
    ...
    private static String systemInstruction() {
        return "당신은 한국어 개인 비서 입력 분석기다. 사용자의 문장을 일정(SCHEDULE), 할 일(TASK), 메모(MEMO) 중 하나로 분류하고 JSON만 반환한다. "
                ...
                + "명시되지 않은 정보는 null 또는 기본값으로 두고 사실을 만들지 않는다. 제목은 핵심 행동이나 일정명만 짧게 작성한다.";
    }
    ...
    public static final class AnalysisResult {
        ...
        AnalysisResult(ParsedWorkItem item, boolean privacyMasked) {
            this.item = item;
