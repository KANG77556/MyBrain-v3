...
/** 클라우드 AI 호출 전 네트워크와 GPT·Gemini 합산 월간 비용 한도를 검사합니다. */
public final class AiCloudPolicy {
    ...
    public static Decision evaluate(Context context, String provider) {
        ...
        AiUsageStore.CombinedSummary summary = AiUsageStore.loadCombined(context);
        long spent = summary.monthlyEstimatedCostWon;
        if (budget.budgetEnabled && budget.blockAtLimit && spent >= budget.monthlyLimitWon) {
            return Decision.blocked(
                    "GPT·Gemini 합산 월간 비용 한도에 도달해 기기 분석으로 전환했습니다.",
                    budget, spent, spent);
        }

        boolean warning = budget.budgetEnabled && spent >= budget.warningAmountWon();
        String message = warning ? "GPT·Gemini 합산 월간 AI 비용이 경고 기준에 도달했습니다." : "";
        return new Decision(true, warning, message, budget, spent, spent);
    }
