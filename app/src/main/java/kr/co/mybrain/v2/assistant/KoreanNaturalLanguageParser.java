...
public final class KoreanNaturalLanguageParser {
    ...
    public static ParsedWorkItem parse(String rawText, ZoneId zoneId) {
        ...
        boolean hasDateWord = containsAny(lower, "오늘", "내일", "모레", "다음 주", "다음주", "이번 주", "이번주", "월요일", "화요일", "수요일", "목요일", "금요일", "토요일", "일요일", "다음 달", "다음달") || MONTH_DAY.matcher(text).find();
        boolean hasTimeWord = TIME_RANGE.matcher(text).find() || SINGLE_TIME.matcher(text).find() || containsAny(lower, "정오", "점심", "자정");
        ...
        result.confidence = rangeDates != null ? 0.97f : (WorkItemEntity.TYPE_MEMO.equals(result.type) ? 0.62f : (hasDateWord || hasTimeWord ? 0.9f : 0.78f));
        return result;
    }
    ...
    private static LocalDate[] resolveDateRange(String text, LocalDate today) {
        ...
        if (m.find()) {
            ...
            LocalDate end = LocalDate.of(start.getYear(), em, ed);
            if (end.isBefore(start)) end = end.plusYears(1);
            return new LocalDate[]{start, end};
        }
        if (text.contains("다음 달") || text.contains("다음달")) {
    ...
    private static LocalTime[] resolveTimes(String text) {
        String normalized = text.replace("정오", "오후 12시").replace("점심", "오후 12시").replace("자정", "오전 12시");
