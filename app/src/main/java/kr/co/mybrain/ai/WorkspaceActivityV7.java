...
/**
 * MyBrain AI 1.8.9 기본 작업 화면입니다.
 * 기존 홈·목록·달력 기능은 유지하면서 모든 빠른 입력 동작을
 * 개인정보 보호 통합 입력 화면과 앱 내부 음성 인식 화면으로 연결합니다.
 */
public class WorkspaceActivityV7 extends WorkspaceActivityV6 {

    private static final int REQUEST_HOME_VOICE = 7801;
    ...
    /** 이전 버전의 화면 패치가 완료된 뒤 새 입력 동작을 마지막으로 적용합니다. */
    ...
    private void patchButton(Button button, List<View> views) {
        ...
        if ("＋".equals(value)) {
            ...
            button.setContentDescription("개인정보 보호 빠른 입력, 길게 누르면 여러 항목 관리");
        } else if ("🎤".equals(value)) {
            button.setOnClickListener(v ->
                    startActivityForResult(new Intent(this, VoiceCaptureActivity.class), REQUEST_HOME_VOICE));
            button.setContentDescription("음성으로 빠른 입력");
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_HOME_VOICE) {
    ...
    private void openUnifiedInput(List<View> views, String prefill, String source) {
        Intent intent = new Intent(this, UnifiedQuickInputActivityV2.class);
