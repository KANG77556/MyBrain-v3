...
/**
 * MyBrain AI 1.9.8 연속 음성 입력 화면입니다.
 * 문장 종료 뒤 자동으로 다음 듣기를 시작하고, 중간 결과와 최종 결과를 분리합니다.
 */
public class VoiceCaptureActivityV4 extends Activity implements RecognitionListener {
    ...
    private static final int REQUEST_AUDIO_PERMISSION = 11980;
    ...
    private void buildScreen() {
        ...
        TextView guide = text("말을 멈추면 문장을 저장하고 자동으로 다음 문장을 듣습니다. 완료할 때 ‘입력창에 추가’를 누르세요.", 14, MUTED);
        guide.setPadding(0, 0, 0, dp(12));
        root.addView(guide);

        statusText = text("음성 인식을 준비하고 있습니다.", 16, PRIMARY);
        statusText.setGravity(Gravity.CENTER);
        root.addView(statusText, new LinearLayout.LayoutParams(-1, dp(48)));
        ...
        row.addView(close, new LinearLayout.LayoutParams(0, dp(48), 1f));

        removeButton = button("마지막 문장 삭제", Color.TRANSPARENT, MUTED);
        removeButton.setEnabled(false);
        removeButton.setOnClickListener(v -> removeLast());
        row.addView(removeButton, new LinearLayout.LayoutParams(0, dp(48), 1f));
        root.addView(row);
    ...
    /** Galaxy 기본 음성 서비스에 맞춘 한국어 연속 인식 설정입니다. */
    private void prepareRecognizer() {
        ...
        recognizerIntent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS, 1800L);
        recognizerIntent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_POSSIBLY_COMPLETE_SILENCE_LENGTH_MILLIS, 1100L);
        recognizerIntent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_MINIMUM_LENGTH_MILLIS, 900L);
    }
    ...
    private void addFinal(String raw) {
        ...
        if (segments.isEmpty() || !isDuplicate(segments.get(segments.size() - 1), value)) {
            segments.add(value);
        }
        partialText = "";
        listening = false;
        ...
        handler.postDelayed(this::startListening, 420L);
    }

    private void updateResult(Bundle bundle, boolean finalResult) {
        ArrayList<String> values = bundle == null ? null
    ...
    /** 여러 후보 중 내용이 충분하고 앞 문장과 중복되지 않는 후보를 우선 선택합니다. */
    ...
    @Override public void onEndOfSpeech() { listening = false; statusText.setText("문장을 정리하고 있습니다."); }
    ...
    public void onError(int error) {
        ...
        if (error == SpeechRecognizer.ERROR_AUDIO) {
            scheduleRetry("마이크 연결을 다시 확인하고 있습니다.");
            ...
            scheduleRetry("잘 듣지 못해 다시 듣고 있습니다.");
        }
    }
