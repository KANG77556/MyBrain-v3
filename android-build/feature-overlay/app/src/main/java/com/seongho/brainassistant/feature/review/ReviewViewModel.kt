...
private fun ReviewItemUi.isValid(): Boolean =
    title.isNotBlank() && when (type) {
        ...
        ItemType.D_DAY -> dueAt.parseInstantOrNull() != null
        else -> true
    }
...
class ReviewViewModel(
    ...
    private fun updateItem(id: String, transform: (ReviewItemUi) -> ReviewItemUi) {
        _state.update { current -> current.copy(items = current.items.map { if (it.localId == id) transform(it) else it }, message = null) }
    }
    ...
    private fun save() = viewModelScope.launch {
        ...
        if (!current.isValid) {
            _state.update { it.copy(message = "제목과 일정 시간을 확인해 주세요.") }
            return@launch
        }
